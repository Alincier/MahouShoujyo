using MahouShoujyo.Content.Items.Consumables.BOSS_Summon;
...
namespace MahouShoujyo.Common.Systems
{
	...
	public class ModIntegrationsSystem : ModSystem
	{
		...
		private void DoBossChecklistIntegration() {
			...
			if (bossChecklistMod.Version < new Version(2, 1)) {
				return;
			}
			...
			string internalName = "ConciousBoss";

			// Value inferred from boss progression, see the wiki for details
			float weight = 7.01f;

			// Used for tracking checklist progress
			Func<bool> downed = () => DownedBossSystem.downedConciousBoss;

			// The NPC type of the boss
			int bossType = ModContent.NPCType<Majo_Consciousness>();

			// The item used to summon the boss with (if available)
			int spawnItem = ModContent.ItemType<DespairSoul>();

			// "collectibles" like relic, trophy, mask, pet
			List<int> collectibles = new List<int>()
			{
				ModContent.ItemType<Majo_ConsciousnessRelic>(),
				//ModContent.ItemType<Content.Pets.>(),
				ModContent.ItemType<Majo_ConsciousnessTrophy>(),
				//ModContent.ItemType<Content.Items.Armor.Vanity.>()
			};

			// By default, it draws the first frame of the boss, omit if you don't need custom drawing
			// But we want to draw the bestiary texture instead, so we create the code for that to draw centered on the intended location
			var customPortrait = (SpriteBatch sb, Rectangle rect, Color color) => {
				Texture2D texture = ModContent.Request<Texture2D>("MahouShoujyo/Content/NPCs/BOSSes/Majo_Consciousness/Majo_Consciousness_Preview").Value;
				Vector2 centered = new Vector2(rect.X + (rect.Width / 2) - (texture.Width / 2), rect.Y + (rect.Height / 2) - (texture.Height / 2));
				sb.Draw(texture: texture, position: centered, color:color);
			};
			...
			bossChecklistMod.Call(
				...
				new Dictionary<string, object>() {
                    ...
                    ["spawnInfo"] =Language.GetText("Mods.MahouShoujyo.NPCs.Majo_Consciousness.spawnInfo"),
