...
namespace MahouShoujyo.Content.Projectiles
{
    public class RedSpearBullet : ModProjectile
    {
        ...
        Vector2[] pos_old;
        Vector2[] vel_old;
        private Texture2D tex = ModContent.Request<Texture2D>("MahouShoujyo/Content/Projectiles/RedSpearBullet").Value;
        ...
        public override void AI()
        {
            ...
            if (returning)
            {
                if (!returning_sound)
                {
                    SoundEngine.PlaySound(new SoundStyle($"MahouShoujyo/Radio/Sound/spear_return"), Projectile.Center, soundInstance =>
                    {
            ...
            if (Main.myPlayer == Projectile.owner)
            {
                ...
                else
                {
                    if (!released) 
                    {
                        ...
                        SoundEngine.PlaySound(new SoundStyle($"MahouShoujyo/Radio/Sound/spear_releasted"), Projectile.Center, soundInstance => {
                            // The SoundUpdateCallback can be inlined if desired, such as in this example.
                            soundInstance.Position = Projectile.Center;
                            return tracker.IsActiveAndInGame();
                        });
        ...
        public override bool OnTileCollide(Vector2 oldVelocity)
        {
            ...
            {
                inserted = true;
                SoundEngine.PlaySound(new SoundStyle($"MahouShoujyo/Radio/Sound/spear_insert"), Projectile.Center, soundInstance => {
                    // The SoundUpdateCallback can be inlined if desired, such as in this example.
                    soundInstance.Position = Projectile.Center;
                    return tracker.IsActiveAndInGame();
                });
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            if (ve.Count >= 3)
            {
                gd.Textures[0] = ModContent.Request<Texture2D>("MahouShoujyo/Content/Assets/MidLight").Value;//获取刀光的拖尾贴图
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            if (released && !inserted)
            {
                ...
                SoundEngine.PlaySound(new SoundStyle($"MahouShoujyo/Radio/Sound/spear_puncture"), Projectile.Center, soundInstance => {
                    // The SoundUpdateCallback can be inlined if desired, such as in this example.
                    soundInstance.Position = Projectile.Center;
                    return tracker.IsActiveAndInGame();
                });
