...
{
    public class combo0 : ModProjectile
    {
        ...
        Vector2[] pos_old;
        Vector2[] vel_old;
        private Texture2D tex = ModContent.Request<Texture2D>("MahouShoujyo/Content/Projectiles/Blue_sword").Value;
        Player player => Main.player[Projectile.owner];
        //public override string Texture => "MahouShoujyo/Content/Items/RangedWeapon/ColorFive.png";
        ...
        {
            Projectile.width = 96; // The width of projectile hitbox
            ...
            //离中心点的手柄位置,剑身长度为64像素
            root0=new Vector2[2] { new Vector2(-16f, -16f), new Vector2(16f, -16f) };//斜后方
            rotation0 =new float[2] { MathHelper.ToRadians(-135f), MathHelper.ToRadians(-45f) };
            length0 = 1f;
            root1 =new Vector2[2] { new Vector2(0f, -16f), new Vector2(0f, -16f) };//头顶
            rotation1 =new float[2] { MathHelper.ToRadians(-90f), MathHelper.ToRadians(-90f) };
            length1 = 1f;
            root2 = new Vector2[2] { new Vector2(-16f, 48f), new Vector2(16f, 48f) };//斩落位置
            rotation2 =new float[2] { MathHelper.ToRadians(15f), MathHelper.ToRadians(-195f) };
            length2 = 1f;
            //阶段帧数
            period1 = 20;
            period2 = 40;
        ...
        public override void AI()
        {
            if (Projectile.timeLeft <=120)
            {
                ...
                //MahouShoujyo.push(Projectile.Center+root*scale+Projectile.velocity*scale, Projectile.velocity*length, frame_tail, ref pos_old, ref vel_old);
                return;
            }
            ...
            Projectile.width = 96*scale; // The width of projectile hitbox
            Projectile.height = 160*scale; // The height of projectile hitbox
            ...
            Vector2 position = Main.player[Projectile.owner].Center+new Vector2(-48f*scale*(direction*2-1), -32f*scale);
            ...
            if (Projectile.ai[0] < period1)
            {
                ...
                Projectile.damage = (int)(damage*damage_bonus);
            ...
            pos_old[0] = root*scale+Projectile.velocity*scale;
            vel_old[0] = Projectile.velocity*length;
            //MahouShoujyo.push(root*scale+Projectile.velocity*scale, Projectile.velocity*length, frame_tail, ref pos_old, ref vel_old);
        }
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            if (ve.Count >= 3)
            {
                gd.Textures[0] = ModContent.Request<Texture2D>("MahouShoujyo/Content/Assets/BlueSwordLight").Value;//获取刀光的拖尾贴图
            ...
            MahouShoujyo.draw_Center(
                ...
                scale_X: 1f * length * scale,
                scale_Y: 1f  * scale,
                (direction==0) ? SpriteEffects.None : SpriteEffects.FlipVertically, layerDepth: 0);
            if (Projectile.timeLeft>120) 
            {                
                Dust.NewDustDirect(Projectile.Center + root+(2*Projectile.velocity)*scale,
