...
namespace MahouShoujyo.Content.Projectiles
{
    ...
    {
        private Texture2D tex = ModContent.Request<Texture2D>("MahouShoujyo/Content/Projectiles/HeartArrow").Value;
        int frame_tail;
        Vector2[] pos_old;
        Vector2[] vel_old;
        ...
        public override void AI()
        {
            ...
            if (Main.player[Projectile.owner].HasBuff<MagicGirlPover>())
            {
                ...
                closestNPC = MahouShoujyo.FindClosestNPC(maxDetectRadius, Projectile.Center);
                focus_strenth = Math.Min(16, Projectile.ai[0]-5);
                if (closestNPC != null)
                {
                    // If found, change the velocity of the projectile and turn it in the direction of the target
                    // Use the SafeNormalize extension method to avoid NaNs returned by Vector2.Normalize when the vector is zero
                    Projectile.velocity = MahouShoujyo.focus(Projectile.Center, Projectile.velocity, closestNPC.Center, projSpeed, focus_strenth);// (Projectile.velocity+focus_strenth*(closestNPC.Center - Projectile.Center).SafeNormalize(Vector2.Zero)).SafeNormalize(Vector2.Zero) * projSpeed;
                    Projectile.rotation =directed +  Projectile.velocity.ToRotation();
                }
            }
            ...
            //记录轨迹
            //if (Main.time % 2 ==0)
            MahouShoujyo.push(Projectile.Center, Projectile.velocity, frame_tail,ref pos_old,ref vel_old);
        }
